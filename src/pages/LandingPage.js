...
// --- 1. QORA LENTA (JARAYONLAR) ---
const featuresData = [
  { title: "Karzinkalar qat'iy nazorati", desc: "Ertalabki karzinka tarqatish elektronlashadi. Hech bir karzinka hisobsiz ketmaydi, bog'bonlar balansi avtomat yuritiladi.", icon: <Box size={28} /> },
  { title: "Kechki tezkor qabul", desc: "Og'irlik, narx va qaytgan karzinkalar bir marta planshetga kiritiladi. Hisoblagich ishga tushib o'zi yakuniy summani chiqaradi.", icon: <Clock size={28} /> },
  { title: "Avtomat Buxgalteriya", desc: "Bog'bon mahsulotlari uchun pul tarqatish endi ko'p vaqt olmaydi. Kimning qanchaga mahsulot topshirgani va jarimalari tizimda aniq ko'rsatiladi.", icon: <Calculator size={28} /> },
  { title: "Xolodelnik", desc: "Xolodelnikdagi har bir partiyaning qachon kirgani, saqlanish muddati va harorati tizim orqali to'liq nazorat qilinadi.", icon: <Warehouse size={28} /> },
  { title: "Chet'elga Eksport Logistikasi", desc: "Yuk mashinalariga ortilgan mahsulotning kuzatuvi, nakladnoy va bojxona hujjatlarini avtomat shakllantirish imkoniyati.", icon: <Truck size={28} /> },
  { title: "Shaffof hisob-kitob", desc: "Bog'bonlar bilan hisob-kitoblar aniq yuritiladi. Kim, qachon, qancha mahsulot topshirgani haqida to'liq va aniq bazaga yig'iladi.", icon: <ShieldCheck size={28} /> }
];
const carouselItems = [...featuresData, ...featuresData, ...featuresData];

// --- 2. APPLE USLUBIDAGI KARUSEL ---
const benefitsData = [
  { title: "Buxgalter shart emas", desc: "Tizim shunchalik oddiyki, undan foydalanish uchun hisob-kitob ilmi kerak emas. Ixtiyoriy xodim bemalol ma'lumotlarni boshqara oladi.", icon: <Users size={32} />, bgImage: '/assets/buxgalter.jpg' },
  { title: "Bir zumda hisobot", desc: "Qabul tugashi bilanoq jami tonna va summani ko'rasiz. Bir tugma bilan hisobotni printerdan chiqarib, ma'lumotlarni arxivlash mumkin.", icon: <Printer size={32} />, bgImage: '/assets/hisob.jpg' },
  { title: "Birgina planshet yetarli", desc: "Endi ishingiz uchun bir nechta daftar kerak emas, birgina planshet yoki notebook jamiki mahsulotingizni boshqaradi. Planshet orqali inson xatolari to'liq nolga tushiriladi.", icon: <MonitorSmartphone size={32} />, bgImage: '/assets/planshet.webp' },
  { title: "Tarix va Statistika", desc: "Tarifingiz tugasa ham ma'lumotlar saqlanib qoladi. Qaysi bog'bon qancha yuk topshirganini yillab orqaga qaytib ko'rishingiz mumkin.", icon: <History size={32} />, bgImage: '/assets/data.jpg' }
];
const infiniteBenefits = [...benefitsData, ...benefitsData, ...benefitsData];

// --- 3. TARIFLAR ---
const pricingData = [
  { title: "Go", desc: "Kichik hajmda ishlaydiganlar uchun", price: "689 $", period: "/oy",
    features: [
  ...
  { title: "Pro", desc: "O'rta va yirik brokerlik tizimlari uchun", price: "899 $", period: "/oy",
    features: [
      { text: "Karzinka nazorati", included: true },
      { text: "Buxgalter xizmati", included: true },
      { text: "Cheklangan arxiv", included: true },
  ...
  { title: "Ultra", desc: "Katta logistika va kompaniyalar uchun", price: "1399 $", period: "/oy",
    features: [
      { text: "Karzinka nazorati", included: true },
      { text: "Buxgalter xizmati", included: true },
...
const HosilimLanding = () => {
  ...
  // 🚀 OPTIMALLASHTIRISH: ekran kengligini bir marta o'lchaymiz (layout thrashing yo'q)
  ...
  const isOrbitHeld = useRef(false);
  ...
  // Ekran o'lchami (resize'da bir marta yangilanadi)
  useEffect(() => {
  ...
  // 🚀 OPTIMALLASHTIRISH: orbita ekrandan chiqsa, burchak yangilanmaydi (CPU tejaladi)
  useEffect(() => {
    const node = orbitWrapRef.current;
    if (!node) return;
    const io = new IntersectionObserver(
      ([entry]) => setOrbitVisible(entry.isIntersecting),
      { threshold: 0.1 }
    );
    io.observe(node);
    return () => io.disconnect();
  }, []);
  // 🚀 ORBITA ANIMATSIYASI (ko'rinmasa yoki ushlab turilsa — to'xtaydi)
  useEffect(() => {
    const animateOrbit = () => {
      if (orbitVisible && !isOrbitHeld.current) {
  ...
  // 🚀 OPTIMALLASHTIRISH: window.innerWidth o'rniga isDesktop ishlatiladi
  const getCardStyle = (index) => {
    const angle = (orbitAngle + index * 120) * (Math.PI / 180);
    const radiusX = isDesktop ? 350 : 180;
    const radiusZ = isDesktop ? 150 : 80;
  ...
  return (
    <div
      className="min-h-screen font-sans overflow-x-hidden relative bg-cover bg-center md:bg-fixed text-gray-100"
        ...
        <div className="relative z-10 flex flex-col min-h-screen">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
              <div className="flex items-center gap-2 cursor-pointer" onClick={() => setIsPricingPageOpen(false)}>
                <div className="w-10 h-10 bg-green-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-green-500/30">
                  <img src="/logo-white.png" alt="Hosilim agro platforma logotipi" className="w-6 h-6" width="24" height="24" loading="lazy" />
                </div>
          ...
          <main className="flex-1 w-auto flex flex-col justify-center py-10 relative overflow-hidden">
            <div className="text-center mb-8 px-4">
              <h1 className="text-3xl md:text-5xl font-black mb-4 tracking-tight drop-shadow-md text-white">Tarifni tanlang</h1>
            ...
            >
              <div ref={pTrackRef} className="flex gap-4 sm:gap-6 w-max will-change-transform py-4 items-center px-4 sm:px-0">
                {infinitePricing.map((item, idx) => {
                  ...
                  const checkClasses = "text-green-500";
                  const uncheckClasses = "text-white/30";
                  ...
                  return (
                    <article key={idx} className={`w-[85vw] sm:w-[350px] md:w-[450px] min-h-[450px] md:min-h-[500px] flex-shrink-0 rounded-[32px] p-6 md:p-10 pointer-events-none select-none text-left flex flex-col backdrop-blur-xl shadow-xl border ${boxClasses}`}>
                      <div className="flex flex-col mb-8">
                        <h2 className="text-2xl sm:text-3xl font-black mb-2">{item.title}</h2>
                      ...
                      <ul className="space-y-3 sm:space-y-4 mb-8 flex-1">
                        {item.features.map((feat, i) => (
                          <li key={i} className={`flex gap-3 items-center ${feat.included ? '' : 'opacity-50'}`}>
                            {feat.included
                              ? <CheckCircle2 size={20} aria-hidden="true" className={`sm:w-6 sm:h-6 shrink-0 ${checkClasses}`} />
                              : <X size={20} aria-hidden="true" className={`sm:w-6 sm:h-6 shrink-0 ${uncheckClasses}`} />}
            ...
            <div className="flex items-center justify-center mt-6 z-20 relative">
              <div className="flex items-center gap-3 backdrop-blur-md border px-4 py-2.5 rounded-full bg-black/40 border-white/10">
                {pricingData.map((_, i) => (
                  <button key={i} onClick={() => jumpToPDot(i)} aria-label={`${i + 1}-tarifga o'tish`} className={`h-2 rounded-full transition-all duration-300 ${pActiveDot === i ? 'w-8 bg-green-500' : 'w-2 bg-white/30 hover:bg-white/50'}`} />
                ))}
                <div className="w-[1px] h-4 mx-1 bg-white/20"></div>
                <button onClick={() => setPIsPaused(!pIsPaused)} aria-label={pIsPaused ? "Karuselni davom ettirish" : "Karuselni to'xtatish"} className="w-6 h-6 flex items-center justify-center rounded-full transition-colors text-white/70 hover:text-white">
      ...
      <div className="relative z-10 flex flex-col pt-[90px] sm:pt-[100px]">
        ...
        <header className="fixed top-0 w-full z-50 flex flex-col">
          <div className={`py-3 transition-colors duration-500 border-b ${scrolled ? 'bg-black/60 backdrop-blur-xl shadow-lg border-white/10' : 'bg-white/10 backdrop-blur-md border-white/20'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 sm:gap-8">
                <nav aria-label="Asosiy navigatsiya" className="hidden md:flex items-center gap-6">
                  {navItems.map(item => (
                    <button
                      key={item.id}
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  <button className="md:hidden p-2 text-white/80 hover:text-white transition-colors" aria-label="Menyuni ochish/yopish" aria-expanded={mobileOpen} onClick={() => setMobileOpen(!mobileOpen)}>
          ...
          {mobileOpen && (
            <div className="md:hidden w-full backdrop-blur-xl border-b shadow-2xl p-4 absolute top-full left-0 mt-0.5 bg-gray-900/95 border-white/10">
              <nav aria-label="Mobil navigatsiya" className="flex flex-col gap-4">
                {navItems.map(item => (
                  <button
                    key={item.id}
                    onClick={() => {
                      if (item.id === 'pricing') { setIsPricingPageOpen(true); setMobileOpen(false); }
                      else { scrollToSection(item.id); setMobileOpen(false); }
                    }}
        ...
        <section className="pb-16 sm:pb-20 px-4 text-center relative min-h-[90vh] flex flex-col justify-center items-center overflow-hidden">
          <div className="relative z-10 w-full flex flex-col items-center">
            ...
            <button onClick={handleAuth} aria-label="Hosilim platformasini hoziroq boshlash" className="px-8 py-4 bg-green-600 text-white font-bold rounded-full shadow-[0_0_40px_rgba(22,163,74,0.4)] flex items-center gap-3 hover:bg-green-500 transition-all hover:-translate-y-1 hover:shadow-[0_0_60px_rgba(22,163,74,0.6)] text-base sm:text-lg border border-green-400/50 mb-4 sm:mb-10">
              Hoziroq boshlash <ArrowUpRight size={24} aria-hidden="true" />
            </button>

            {/* 3D ORBITA (ekran tashqarisida to'xtaydi) */}
            <div
              ref={orbitWrapRef}
            ...
            >
              {/* 1-Karta: Karzinkalar */}
              <div className={orbitCardBaseClasses} style={getCardStyle(0)}>
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-blue-500/20 flex items-center justify-center border border-blue-500/30 text-blue-400"><Box size={28} aria-hidden="true" /></div>
                  <div className="text-left">
                    <p className="text-xs font-bold text-gray-300 uppercase tracking-wider">Karzinkalar</p>
                    <p className="text-2xl font-black text-white">4,250 <span className="text-sm font-medium text-blue-400">dona</span></p>
                  </div>
                </div>
                <div className="space-y-3 mt-auto">
                  <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden"><div className="w-[70%] h-full bg-blue-500 rounded-full"></div></div>
                  <p className="text-xs font-medium text-gray-300 text-left">Bugungi tarqatilgan karzinkalar</p>
                </div>
              </div>

              {/* 2-Karta: Tushum */}
              <div className={orbitCardBaseClasses} style={getCardStyle(1)}>
                <div className="flex justify-between items-start">
                  <div className="w-14 h-14 rounded-2xl bg-green-500/20 flex items-center justify-center border border-green-500/30 text-green-400"><TrendingUp size={28} aria-hidden="true" /></div>
                ...
                <div className="text-left mt-2">
                  <p className="text-xs font-bold text-gray-300 uppercase tracking-widest mb-1">Jami kunlik tushum</p>
                  <p className="text-4xl font-black text-white drop-shadow-md">18.7 <span className="text-xl font-bold text-gray-300">tonna</span></p>
                </div>
              ...
              {/* 3-Karta: Hisob-kitob */}
              <div className={orbitCardBaseClasses} style={getCardStyle(2)}>
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-purple-500/20 flex items-center justify-center border border-purple-500/30 text-purple-400"><Calculator size={28} aria-hidden="true" /></div>
                  <div className="text-left">
                    <p className="text-xs font-bold text-gray-300 uppercase tracking-wider">Hisob-kitob</p>
                ...
                <div className="space-y-4 mt-auto">
                  <div className="flex items-center gap-3">
                    ...
                    <CheckCircle2 size={18} aria-hidden="true" className="text-green-400" />
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-white/10"></div>
                    <div className="flex-1 h-3 bg-white/10 rounded-full"></div>
                    <Clock size={18} aria-hidden="true" className="text-yellow-400" />
                  </div>
                </div>
              </div>
        ...
        <section id="problems" className="py-16 sm:py-20 text-left overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-16">
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-3 sm:mb-4 tracking-tight px-2 drop-shadow-md text-white">Zamon bilan hamnafas bo'ling</h2>
              <p className="text-base sm:text-lg md:text-xl font-medium max-w-2xl mx-auto leading-relaxed px-4 drop-shadow-md text-gray-200">Bu tizim sizni hozirgacha ketqazgan asab va daromadingizga davo bo'lishi mumkin.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 relative z-10">
              <article className="backdrop-blur-xl rounded-3xl p-6 md:p-10 border relative overflow-hidden transition-transform duration-300 ease-in-out hover:-translate-y-2 cursor-pointer group bg-red-500/10 border-red-500/30 shadow-[0_8px_30px_rgb(239,68,68,0.15)] hover:shadow-[0_15px_40px_rgb(239,68,68,0.25)]">
                <div className="absolute top-0 right-0 p-4 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12 opacity-10 text-red-400" aria-hidden="true"><XOctagon size={100} className="sm:w-[120px] sm:h-[120px]" /></div>
                <h3 className="text-xl sm:text-2xl font-black mb-5 sm:mb-6 flex items-center gap-2 sm:gap-3 drop-shadow-sm text-white"><XOctagon className="w-6 h-6 sm:w-8 sm:h-8 text-red-400" aria-hidden="true" /> Hozirgi usul (Xavfi)</h3>
              ...
              <article className="backdrop-blur-xl rounded-3xl p-6 md:p-10 border relative overflow-hidden transition-transform duration-300 ease-in-out hover:-translate-y-2 cursor-pointer group bg-green-500/20 border-green-500/40 text-white shadow-[0_8px_30px_rgb(34,197,94,0.2)] hover:shadow-[0_15px_40px_rgb(34,197,94,0.3)]">
                <div className="absolute top-0 right-0 p-4 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-12 opacity-20 text-green-300" aria-hidden="true"><CheckCircle2 size={100} className="sm:w-[120px] sm:h-[120px]" /></div>
                <h3 className="text-xl sm:text-2xl font-black mb-5 sm:mb-6 flex items-center gap-2 sm:gap-3 drop-shadow-md text-white"><CheckCircle2 className="w-6 h-6 sm:w-8 sm:h-8 text-green-300" aria-hidden="true" /> Hosil tizimi (Foydasi)</h3>
            ...
            <div className="cursor-grab active:cursor-grabbing touch-pan-y"
              ...
              <div ref={trackRef} className="flex gap-4 sm:gap-6 w-max will-change-transform px-4">
                {carouselItems.map((item, index) => (
                  <article key={index} className="w-[280px] sm:w-[350px] flex-shrink-0 backdrop-blur-xl rounded-2xl p-6 sm:p-8 border shadow-lg pointer-events-none select-none text-left bg-white/10 border-white/20">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center mb-5 sm:mb-6 border bg-green-500/20 text-green-400 border-green-400/30" aria-hidden="true">{item.icon}</div>
              ...
              {infiniteBenefits.map((item, idx) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-start">
                    <div className="scale-75 sm:scale-100 origin-top-left" aria-hidden="true">{item.icon}</div>
          ...
          <div className="flex items-center justify-center mt-6 relative z-10">
            <div className="flex items-center gap-3 backdrop-blur-md border px-4 py-2.5 rounded-full bg-black/40 border-white/10">
              {benefitsData.map((_, i) => (
                <button key={i} onClick={() => jumpToDot(i)} aria-label={`${i + 1}-afzallikka o'tish`}
              ...
              <div className="w-[1px] h-4 mx-1 bg-white/20"></div>
              <button onClick={() => setBIsPaused(!bIsPaused)} aria-label={bIsPaused ? "Karuselni davom ettirish" : "Karuselni to'xtatish"} className="w-6 h-6 flex items-center justify-center rounded-full transition-colors text-white/70 hover:text-white">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex flex-col min-h-[160px] md:min-h-[200px] mb-12 relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-auto md:justify-between items-center md:items-end pt-4 md:pt-0 w-full">
                ...
                <a
                  href="tel:+998917770137"
                  className="px-6 py-3.5 backdrop-blur-md border font-bold rounded-xl transition-transform text-sm sm:text-base whitespace-nowrap w-full max-w-[280px] sm:max-w-none sm:w-auto mx-auto sm:mx-0 flex items-center justify-center hover:-translate-y-1 bg-white/10 text-white border-white/20 hover:bg-white/20"
                >
                  +998 91 777 01 37
                </a>
              </div>
            ...
            <div className="border-t pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm font-medium relative z-20 text-center md:text-left border-white/10 text-gray-300">
              <div className="flex items-center justify-center md:justify-start gap-3 w-full md:w-auto cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                <div className="w-8 h-8 bg-green-500 rounded-lg flex items-center justify-center text-white shadow-lg shadow-green-500/30">
                  <img src="/logo-white.png" alt="Hosilim.uz logotipi" className="w-5 h-5" width="20" height="20" loading="lazy" />
                </div>
